...
def TICKET_CREATE(st: State, client: str, model_id: str, round_: int, deposit: int, expiry_slot: int) -> Ticket:
    ...
    if not ok:
        # race: balance changed after admission; just drop
        return None
...
def ENTITLEMENT_TRANSFER(st: State, acct: str, model_id: str, amount: int):
    # guard against replays or stale mempool
...
def ADVANCE_ROUND(st: State, model_id: str, round_: int, global_cid: str,
                  included: List[int], scores: List[float], refunds: List[int]) -> bool:
    """
    Idempotent round finalization:
      - If current round != tx.round, ignore (stale or duplicate).
      - Only process tickets of exactly (model_id, round_).
      - Mark refunds/forfeits once using ticket.settled flag.
      - Return True iff we actually advanced.
    """
    cur = st.get_round_index(model_id)
    if cur != round_:
        return False  # stale finalize; round already moved (or too early)
    ...
    # forfeits for others (unsettled tickets of this round)
    for t in st.tickets_for(model_id, round_):
        if t.settled:
            continue
        t.forfeited = True
        ...
        # 'burn' => do nothing (deposit disappears); default to treasury if unknown policy

    # consume exactly R (cap by balance)
...
def apply_tx(st: State, tx: Dict[str, Any]) -> bool:
    """Return True if the tx mutated state (used to ignore stale tx)."""
    t = tx["type"]
    if t == "ListingTx":
        # Idempotent: ignore duplicate listings
        mid = tx["model_id"]
        if mid in st.models:
            return False
        ...
        return ADVANCE_ROUND(st, tx["model_id"], tx["round"], tx["global_cid"],
    ...
    def run(self):
        ...
        while self.running:
            ...
            if revealed:
                ...
                # wait until the chain actually moves to next round
                # (prevents submitting another finalize for the same r)
                t0 = time.time()
                while time.time() - t0 < 3.0:
                    with self.state.lock():
                        if self.state.models[self.model_id].round_index > r:
                            break
                    time.sleep(0.1)
            else:
                time.sleep(0.2)
...
class Client(threading.Thread):
    ...
    def _my_tickets(self, round_: int) -> list[int]:
        with self.state.lock():
            return [t.ticket_id for t in self.state.tickets_for(self.model_id, round_) if t.owner == self.acct]
    ...
    def run(self):
        ...
        while self.running:
            ...
            tids = self._my_tickets(r)   # <-- fixed call
            if not tids:
                time.sleep(0.2)
                continue
            tid = tids[-1]
