...
def main():
    ...
    ap.add_argument("--client_sponsor", type=int, default=0, help="amount Lister transfers to each client at start")
    ...
    if args.client_sponsor > 0:
        with state.lock():
            need = args.client_sponsor * args.clients
            assert state.balances.get("Lister", 0) >= need, "Lister lacks balance to sponsor clients"
            for c in range(args.clients):
                acct = f"C{c+1}"
                state.balances["Lister"] -= args.client_sponsor
                state.balances[acct] = state.balances.get(acct, 0) + args.client_sponsor
        ...
        for i in range(args.clients):
            ...
            if elig > 0:
                ...
                validators[0].submit_tx({"type":"ClaimTx","acct":acct,"model_id":model_id,
                                         "amount":elig,"nonce":nonce,"fee":0,"created_slot":state.slot})
    time.sleep(0.6)
    ...
    with state.lock():
        ...
        if state.round_log:
            ...
            for r in state.round_log[-5:]:
                print(f"  model={r['model']} round={r['round']} pool={r['pool']} "
                      ...
                      f"start={r['start_slot']} finalize={r['finalize_slot']} dur={r['round_dur_slots']} slots")
        ...
        tlat = state.metrics["tx_lat_slots_by_type"]
        intent_lat_med = _median(tlat["UpdateIntentTx"])
        publish_lat_med = _median(tlat["UpdatePublishTx"])
        finalize_lat_med = _median(tlat["GlobalUpdateTx"])
        ...
        if args.csv_out:
            row = {
                ...
                "intent_lat_slots_median": int(intent_lat_med),
                "publish_lat_slots_median": int(publish_lat_med),
                "finalize_lat_slots_median": int(finalize_lat_med),
