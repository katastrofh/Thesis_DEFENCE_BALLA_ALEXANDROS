...
class Client(threading.Thread):
    ...
    def _maybe_claim(self):
        ...
        txc = {"type": "ClaimTx", "acct": self.acct, "model_id": self.model_id,
               "amount": elig, "nonce": nonce, "fee": 2, "created_slot": created_slot}
        self.submit_tx(txc)
    ...
    def run(self):
        ...
        while self.running:
            ...
            txi = {"type": "UpdateIntentTx", "client": self.acct, "model_id": self.model_id,
                   "round": r, "deposit": dep, "fee": 1, "created_slot": created_slot}
            self.submit_tx(txi)
            time.sleep(0.4)
            ...
            with self.state.lock(): created_slot_p = self.state.slot
            txp = {"type": "UpdatePublishTx", "client": self.acct, "ticket_id": tid,
                   "update_cid": update_cid, "fee": 1, "created_slot": created_slot_p}
            self.submit_tx(txp)
